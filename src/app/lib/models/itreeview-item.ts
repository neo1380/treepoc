...
export interface TreeviewSelection {
  checkedItems: ITreeviewItem[];
  uncheckedItems: ITreeviewItem[];
}
...
export class ITreeviewItem {
  ...
  private internalChildren: ITreeviewItem[];
  text: string;
  value: any;

  constructor(item: ITreeItem, autoCorrectChecked = false) {
    if (isNil(item)) {
    ...
    if (!isNil(item.children) && item.children.length > 0) {
      this.children = item.children.map(child => {
        ...
        return new ITreeviewItem(child);
      });
    }
  ...
  get children(): ITreeviewItem[] {
    return this.internalChildren;
  }

  set children(value: ITreeviewItem[]) {
