...
import { TreeviewHelper } from '../../helpers/itreeview-helper';
import { TreeviewEventParser } from '../../helpers/itreeview-event-parser';

class FilterTreeviewItem extends ITreeviewItem {
  private readonly refItem: ITreeviewItem;
  constructor(item: ITreeviewItem) {
    super({
      text: item.text,
      value: item.value,
...
@Component({
  selector: 'itreeview',
  templateUrl: './itreeview.component.html',
  styleUrls: ['./itreeview.component.scss']
})
export class TreeviewComponent implements OnChanges, OnInit {
  ...
  allItem: ITreeviewItem;
  filterText = '';
  filterItems: ITreeviewItem[];
  ...
  constructor(
    ...
    this.allItem = new ITreeviewItem({ text: 'All', value: undefined });
  }
  ...
  onItemCheckedChange(item: ITreeviewItem, checked: boolean): void {
    if (item instanceof FilterTreeviewItem) {
      item.updateRefChecked();
    }
  ...
  private generateSelection(): void {
    let checkedItems: ITreeviewItem[] = [];
    let uncheckedItems: ITreeviewItem[] = [];
    if (!isNil(this.items)) {
  ...
  private updateFilterItems(): void {
    if (this.filterText !== '') {
      const filterItems: ITreeviewItem[] = [];
  ...
  private filterItem(item: ITreeviewItem, filterText: string): ITreeviewItem {
